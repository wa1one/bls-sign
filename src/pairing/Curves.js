...
class Curve {
    ...
    pointFactory(rand) {
        if (rand instanceof CryptoRandom) {
            return this.G.multiply(
                ExNumber.construct(2 * this.bn.p.bitLength())
            )
        } else {
...
class Curve2 extends Curve {
    constructor(E) {
        ...
        if (E instanceof Curve) {
            ...
            if (E.bn.m == 256)
                this.b = new Field2(E.bn.p, bigInt(3)).divide(
                    new Field2(E.bn.p, bigInt(9), bigInt(1), false)
                )
