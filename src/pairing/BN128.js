...
class BN128Fp {
    ...
    isOnCurve() {
        ...
        const right = this.x
            .square()
            .multiply(this.x)
            .add(this.b().multiply(z6)) // x^3 + b * z^6
...
class BN128Fp2 {
    ...
    b() {
        const twinv = BN128Fp2.TWIST.inverse()
        const B_Fp2 = new Fp2(
            twinv.a.multiply(new Field(bigInt(3))),
            twinv.b.multiply(new Field(bigInt(3)))
        )
        return B_Fp2
    }
    ...
    mulByP() {
        const rx = BN128Fp2.TWIST_MUL_BY_P_X.multiply(
            this.x.frobeniusMap(bigInt(1))
        )
        const ry = BN128Fp2.TWIST_MUL_BY_P_Y.multiply(
            this.y.frobeniusMap(bigInt(1))
        )
        const rz = this.z.frobeniusMap(bigInt(1))
