...
class PairingCheck {
    ...
    static get PAIRING_FINAL_EXPONENT_Z() {
        return bigInt(4965661367192848881n)
    }

    static get LOOP_COUNT() {
        return bigInt(29793968203157093288n)
    }
    ...
    finalExponentiation(el) {
        ...
        const z = y.frobeniusMap(bigInt(2))
        ...
        const o = l.frobeniusMap(bigInt(1))
        const p = o.multiply(n)
        const q = k.frobeniusMap(bigInt(2))
        const r = q.multiply(p)
        const s = pre.unitaryInverse()
        const t = s.multiply(l)
        const u = t.frobeniusMap(bigInt(3))
        return u.multiply(r)
    }
}
