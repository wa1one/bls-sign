...
const params = require('./bls12381Params')

const { p, _0, _1, B, B_FP2, fp2Params } = params
...
class BLS12Fp {
    ...
    static ZERO = new BLS12Fp(_0, _0, _0)
    static B = B

    neg = () => new BLS12Fp(this.x, this.y.negate(), this.z)
    ...
    toAffine() {
        if (this.isZero()) {
            let zero = BLS12Fp.ZERO
            return new BLS12Fp(zero.x, _1, zero.z) // (0; 1; 0)
        }
    ...
    isOnCurve() {
        ...
        const right = this.x
            .square()
            .multiply(this.x)
            .add(BLS12Fp.B.multiply(z6)) // x^3 + b * z^6
    ...
    eq(o) {
        ...
        if (!(o instanceof BLS12Fp)) return false
    ...
    static create(xx, yy) {
        const x = new Field(xx, p)
        const y = new Field(yy, p)
...
class BLS12Fp2 {
    ...
    neg = () => new BLS12Fp2(this.x, this.y.negate(), this.z)
    ...
    toAffine() {
        if (this.isZero()) {
            let zero = BLS12Fp2.ZERO
            return new BLS12Fp2(zero.x, Fp2.one(fp2Params), zero.z) // (0; 1; 0)
        }
        ...
        return new BLS12Fp2(ax, ay, Fp2.one(fp2Params))
    }
    ...
    isOnCurve() {
        ...
        const right = this.x
            .square()
            .multiply(this.x)
            .add(BLS12Fp2.B_FP2.multiply(z6)) // x^3 + b * z^6
        return left.eq(right)
    }
    ...
    eq(o) {
        if (this === o) return true
        if (!(o instanceof BLS12Fp2)) return false
        if (this.isZero() || o.isZero()) return this.isZero() === o.isZero()
    ...
    static create(aa, bb, cc, dd) {
        const x = new Fp2(aa, bb, fp2Params)
        const y = new Fp2(cc, dd, fp2Params)
        ...
        const pt = new BLS12Fp2(x, y, Fp2.one(fp2Params))
...
module.exports = { BLS12Fp, BLS12Fp2 }
