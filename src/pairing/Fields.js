...
class Field {
    ...
    add(o) {
        return new Field(ExNumber.mod(this.v.add(o.v), Parameters.p))
    }
    multiply(o) {
        if (o instanceof Fp2) {
    ...
    subtract(o) {
        return new Field(ExNumber.mod(this.v.subtract(o.v), Parameters.p))
    }
    square() {
        return new Field(ExNumber.mod(this.v.multiply(this.v), Parameters.p))
    }
    double() {
        return new Field(ExNumber.mod(this.v.add(this.v), Parameters.p))
    }
    inverse() {
        return new Field(this.v.modInv(Parameters.p))
    }
    divide(o) {
        const inv = o.inverse()
        return new Field(ExNumber.mod(this.v.multiply(inv.v), Parameters.p))
    }
    negate() {
        return new Field(this.v.negate().mod(Parameters.p))
    }
    isZero() {
        return this.v.compareTo(bigInt.zero) == 0
    }
    exp(k) {
        const w = this
        for (const i = k.bitLength() - 2; i >= 0; i--) {
            w = w.square()
...
class Fp2 {
    ...
    square() {
        ...
        const ra = this.a
            ...
            .subtract(ab)
            .subtract(ab.multiply(Field.NON_RESIDUE)) // ra = (a + b)(a + NON_RESIDUE * b) - ab - NON_RESIDUE * b
    ...
    multiply(o) {
        ...
        const ra = bb.multiply(Field.NON_RESIDUE).add(aa) // ra = a1 * a2 + NON_RESIDUE * b1 * b2
        const rb = this.a
            .add(this.b)
            .multiply(o.a.add(o.b))
            .subtract(aa)
            .subtract(bb) // rb = (a1 + b1)(a2 + b2) - a1 * a2 - b1 * b2
    ...
    frobeniusMap(power) {
        ...
        const rb = Fp2.FROBENIUS_COEFFS_B[power.mod(bigInt(2))].multiply(this.b)
...
class Fp6 {
    ...
    frobeniusMap(power) {
        ...
        const rb = Fp6.FROBENIUS_COEFFS_B()[power.mod(6).toJSNumber()].multiply(
            this.b.frobeniusMap(power)
        )
        const rc = Fp6.FROBENIUS_COEFFS_C()[power.mod(6).toJSNumber()].multiply(
...
class Fp12 {
    ...
    frobeniusMap(power) {
        ...
        const rb = this.b
            .frobeniusMap(power)
            .multiply(Fp12.FROBENIUS_COEFFS_B[power.mod(12)])
...
class Field2 {
    ...
    neg() {
        return new Field2(
            this.p,
            ExNumber.signum(this.re) !== 0 ? this.p.subtract(this.re) : this.re,
            ExNumber.signum(this.im) !== 0 ? this.p.subtract(this.im) : this.im,
            false
        )
    }
    ...
    halve() {
        return new Field2(
            this.p,
            (ExNumber.testBit(this.re, 0)
                ? this.re.add(this.p)
                : this.re
            ).shiftRight(1),
            (ExNumber.testBit(this.im, 0)
                ? this.im.add(this.p)
                : this.im
            ).shiftRight(1),
            false
    ...
    multiply(v) {
        if (v instanceof Field2) {
            ...
            const newre = this.re.multiply(bigInt(v.toString()))
            ...
            const newim = this.im.multiply(bigInt(v.toString()))
    ...
    square() {
        ...
        if (ExNumber.signum(this.re) === 0) {
            return new Field2(
                this.p,
                this.im.multiply(this.im).negate(),
                _0,
                true
            )
        }
    ...
    divV() {
        ...
        return new Field2(
            this.p,
            (ExNumber.testBit(qre, 0) ? qre.add(this.p) : qre).shiftRight(1),
            (ExNumber.testBit(qim, 0) ? qim.add(this.p) : qim).shiftRight(1),
            false
        )
    }
    ...
    exp(k) {
        ...
        const e = ExNumber.toByteArray(k)
    ...
    sqrt() {
        ...
        const r = this.exp(this.p.multiply(this.p).add(_7).divide(16))
        const r2 = r.square()
        ...
        const invSqrtMinus2 = this.p
            .subtract(_2)
            .modPow(
                this.p.subtract(_1).subtract(this.p.add(_1).divide(4)),
                this.p
            ) // 1/sqrt(-2) = (-2)^{-(p+1)/4}
...
class Field12 {
    ...
    multiply(k) {
        if (bigInt.isInstance(k) || k instanceof Field2) {
            ...
            const b = new Array(this.degree * 2 - 1).fill(
                new Field2(this.bn.p, _0)
            )

            this.split()
            k.split()

            for (const i = 0; i < this.degree; i++) {
                for (const j = 0; j < this.degree; j++) {
            ...
            while (b.length > this.degree) {
                ...
                const top = b.pop()
                for (const i = 0; i < this.degree; i++) {
                    b[exp + i] = b[exp + i].subtract(
                        top.multiply(this.poly_coeffs[i])
                    )
                }
            }
    ...
    inverse() {
        ...
        const poly_div = (a, b) => {
            ...
            for (const i = dega - degb; i > -1; i--) {
                o[i] = o[i].add(temp[degb + i].divide(b[degb]))
                for (const c = 0; c < degb + 1; c++) {
        ...
        const high = this.poly_coeffs
            .map((e) => new Field2(this.bn.p, e))
            .slice()
        ...
        while (deg(low)) {
            ...
            r = r.concat(
                new Array(this.degree + 1 - r.length).fill(
                    new Field2(this.bn.p, _0)
                )
            )
            ...
            for (const i = 0; i < this.degree + 1; i++) {
                for (const j = 0; j < this.degree + 1 - i; j++) {
